...
from typing import TYPE_CHECKING, AsyncIterator, Awaitable, cast
...
# Type aliases for Redis clients
# Note: redis-py's async classes are not generic
type RedisClientType = Redis | RedisCluster
type RedisCommands = AsyncCoreCommands
...
class BaseRedisClient(ABC):
    ...
    async def ahealth_check(self) -> HealthCheckStatus:
        ...
        try:
            # Cast needed: redis-py's ping() returns Union[Awaitable[bool], bool]
            # but we're using async client so it's always Awaitable
