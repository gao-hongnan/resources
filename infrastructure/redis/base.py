...
from typing import TYPE_CHECKING, AsyncIterator, Protocol, cast
...
class AsyncCloseable(Protocol):
    """Protocol for async closeable resources.

    Workaround for outdated types-redis stubs that don't expose aclose().
    The actual redis-py library (v5.0+) uses aclose() as the primary method,
    while close() is deprecated.
    """
